...
#####
FUNDF <- function(value,z=z,DemMap=DemMap) {
	
	o <- NULL
##	print(z)

	elev <- as.data.frame(DemMap,xy=TRUE)
	for(i in 1:nlayers(value)) {
		
		temp <- as.data.frame(value[[i]],xy=TRUE)
		
		print(z[i])
		if (any(temp$x!=elev$x)) {
		...
		temp$ZDEPTH <- z[i]
		temp$TERRAIN_ELEV <- elev$layer
		temp$Z <- temp$TERRAIN_ELEV-temp$ZDEPTH
	#	print(names(value))
	#	print(names(temp))
...
thetadf <- lapply(X=theta,FUN=FUNDF,z=z,DemMap=DemMap)
psidf <- lapply(X=psi,FUN=FUNDF,z=z,DemMap=DemMap)
...
##thetadf <- thetadf[,c("time","x","y","Z","ZDEPTH","TERRAIN_ELEV","value")]
##psidf <- psidf[,c("time","x","y","Z","ZDEPTH","TERRAIN_ELEV","value")]


thetadf <- thetadf[,c("time","x","y","Z","value")]
psidf <- psidf[,c("time","x","y","Z","value")]

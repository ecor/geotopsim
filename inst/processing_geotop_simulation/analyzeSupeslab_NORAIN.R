...
wpath <- '/home/ecor/Dropbox/R-packages/geotopsim_intercomparison/superslab_norain'
		
		####"/home/ecor/Dropbox/R-packages/geotopsim_simulations/geotop_simulation" ######		paste(wpath_pkg,"geotop_simulation",sep="/")

wpath_output <- "/home/ecor/Dropbox/Public/geotop_intercomparison/superslab_NORAIN" 
...
#### DRAFT FUNCTION 
profile.raster.image <- function(df,...) {
	
	
	x <- sort(unique(df$x))
	z <- rev(sort(unique(df$z)))
	
	rows <- data.frame(z=z,r=index(z))
	cols <- data.frame(x=x,c=index(x))
	
	out  <-  df
	out <- merge(out,rows)
	out <- merge(out,cols)
    
	m <- array(as.numeric(NA),c(length(z),length(x)))
	
	for ( i in 1:nrow(out)) {
		
		
		m[out$r[i],out$c[i]] <- out$value[i]
		
	} 
	

	
	########image(m,xlim=range(out$x),ylim=range(out$z),...)
	out <- raster(m,xmn=min(x),xmx=max(x),ymn=min(z),ymx=max(z))
...
for (i in (1:length(file.dattheta_v))) {
	...
	jpgfile <- str_replace(file.dattheta_v[i],".dat",".jpg")
	
	ggout <- ggplot(dfout, aes(x, z, fill = value)) + geom_raster()+scale_fill_gradient(high="blue",low="green")+ggtitle(time.inv[i])
	
	ggsave(filename=jpgfile,plot=ggout)
